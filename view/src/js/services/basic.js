...
$("#gerar_certificado_palestras").submit(function (c) {
    $('.form-load').addClass('show');
    $('button[type="submit"]').prop("disabled", true);
    ...
    $.ajax({
        type: "POST", async: true, data: form.serialize(),
        url: DOMAIN + '/certificados/palestras/emitirCheckVisita',
        success: function (data) {
            window.location.href = DOMAIN + '/certificados/pdf/palestras.php?codigo='+data;
        }
    });
});
