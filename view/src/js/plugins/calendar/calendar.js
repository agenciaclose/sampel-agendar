...
(function($){
    ...
    $('#calendar').jsRapCalendar({
        ...
        onClick:function(y,m,d){
            ...
            jQuery('.prosseguir').prop('disabled', true);

            var current_date = y+'-'+m+'-'+d;
            var area = $('#servico').val();
            var caminho = $('#dados').attr('data-theme');

            $.ajax({
                url: caminho+"/area/horarios",
                method: "POST",
                data: {current_date: current_date, areaID: area },
                success: function(data) {
                    $('#horarios_disponiveis').html(data);
                    checkHour();
                }
            });
...
function checkHour(){
    var selected = [];
    $('#horarios_disponiveis input:checked').each(function() {
    ...
    $('#price_total').text(priceTotal.toLocaleString("pt-BR", { style: "currency" , currency:"BRL"}));
    $('#total').val(priceTotal);

    if( (selected != '') && ($('#termo_servico').is(':checked')) ){
       jQuery('.prosseguir').prop('disabled', false); 
    }else{
       jQuery('.prosseguir').prop('disabled', true);
    }
