...
document.querySelectorAll("[data-numspin").forEach(function(element){
    ...
    var minusBtnVal = '<button class="btn btn-primary minus-btn" type="button"><i class="fa-light fa-minus"></i></button>';
    element.insertAdjacentHTML("beforebegin", minusBtnVal);

    var plusBtnVal = '<button class="btn btn-primary plus-btn" type="button"><i class="fa-light fa-plus"></i></button>';
    ...
    wrapper.querySelector(".plus-btn").addEventListener("click", function(){
        ...
        if (newInputVal <= maxVal) {
            ...
            itemTotal(newInputVal, numInput.getAttribute('data-valor'), numInput.getAttribute('data-item'), numInput.getAttribute('data-validation'), numInput.getAttribute('data-unidades'));
        }
    })
