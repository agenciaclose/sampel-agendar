$(document).ready(function () {
    ...
    $("#login-form").submit(function (c) {
        ...
        $.ajax({
            ...
            url: DOMAIN + '/painel/login/sign',
            success: function (data) {
                $('#info-login').hide();
                if (data == "1") {
                    window.location.href = DOMAIN+'/painel';
                } else {
    ...
    $("#recover-form").submit(function (event) {
        ...
        $.ajax({
            type: "POST", async: true, data: form.serialize(),
            url: DOMAIN + '/painel/login/recover-send',
            success: function (data) {
                ...
                if (result.error === false) {
                    ...
                    $('.login-load').hide();
                    setTimeout(function() { window.location.href = DOMAIN + '/painel/login'}, 3000);
                } else {
