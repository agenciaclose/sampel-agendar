$(document).ready(function () {
	...
	$('#conta_save').submit(function(e){
		...
		$.ajax({
			url: domain + '/painel/saques/conta/salvar',
			data: formData,
			type: 'POST',
			success: function(data){
				if (data === '1') {
					Swal.fire({
						...
						onClose: function () {
							window.location.href = domain + '/painel/saques';
						}
					});
				}else{
    ...
    $('#add_save_saque').submit(function(e){
		...
		$.ajax({
			url: domain + '/painel/saques/conta/save-saque',
			data: formData,
			type: 'POST',
			success: function(data){
				if (data === '0') {
...
function checkV(v){
	...
	$.ajax({
		url: domain + '/painel/saques/conta/check',
		type: 'POST',
		data: {valor: v},
		success: function(data){
