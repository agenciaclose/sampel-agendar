<?php
    ...
    class MinhasInscricoes extends Model
    {
        ...
        {
            $read = new Read();
            $read->FullRead("SELECT vi.id, vi.id_visita AS id_evento, vi.cpf, v.title, v.data_visita AS data_inicio, NULL AS data_fim, v.horario_visita, v.tipo, 'visita' AS origem, vi.codigo AS codigo, vi.presenca
                        FROM visitas_inscricoes AS vi
                        INNER JOIN visitas AS v ON v.id = vi.id_visita 
                        WHERE vi.cpf = :cpf
                        UNION ALL
                        SELECT pp.id, pp.id_palestra AS id_evento, pp.cpf, p.title, p.data_palestra AS data_inicio, p.data_fim, NULL AS horario_visita, NULL AS tipo, 'palestras' AS origem, pp.codigo AS codigo, pp.presenca
